...
def load_payload(code: str):
    ...
    assert path.exists(), f"[ERR] payload not found: {path}"
    return pd.read_pickle(path)  # {'Xp_tr','Xf_tr','Xd_tr','Y_tr', ... 'meta'}


# ===== Dataset (arrays → loader) =====
class MIDataset(Dataset):
    def __init__(self, Xp, Xf, Xd, Y):
        # float32로 맞춰두면 GPU 복사 시 안전
...
class MultiInputLSTM(nn.Module):
    def __init__(self, p_dim, f_dim, d_dim,
                 hidden=96, layers=1,
                 head_hidden=128, out_dim=1, dropout=0.2):
        super().__init__()
...
def get_close_idx(feat: Dict[str, List[str]]) -> int:
    ...
    return cols.index("close") if "close" in cols else 3  # open, high, low, close, volume 가정
...
def main():
    ...
    assert ckpt_path.exists(), f"[ERR] checkpoint not found: {ckpt_path}"
    ...
    with torch.no_grad():
        for b in test_loader:
            ...
            if price_scaler is not None:
                inv = price_scaler.inverse_transform(x_last)  # (B, P)
    ...
    if ROLLING_H1_PLOT:
        ...
        plt.title(f"[Rolling h=1] Absolute Price Path (Code={Code}, target={target_kind})")
